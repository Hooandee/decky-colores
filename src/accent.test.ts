...
import { ACCENTS, DEFAULT_ACCENT, resolveAccent, hexToRgbTriplet } from "./accent";

describe("accent palette", () => {
  it("defaults to blue", () => {
    expect(DEFAULT_ACCENT.id).toBe("blue");
    expect(DEFAULT_ACCENT.hex.toLowerCase()).toBe("#5b8cff");
  });
...
describe("hexToRgbTriplet", () => {
  it("converts a hex to comma-separated components", () => {
    expect(hexToRgbTriplet("#5b8cff")).toBe("91,140,255");
  });
  ...
  it("falls back to the blue triplet for a non-hex value", () => {
    expect(hexToRgbTriplet("var(--x)")).toBe("91,140,255");
  });
});
