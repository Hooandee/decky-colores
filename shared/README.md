...
El driver `htr3212` conserva esos campos para el estado oficial de brillo, encendido y color de respaldo, y añade un bloque `htr3212` con `leftBus`, `rightBus`, `address`, `leftOrder` y `rightOrder`. Los campos opcionales `rgbStartRegister`, `blockWrite`, `pairedWrite` y `explicitInitialization` describen únicamente protocolos demostrados. `automaticActivation` permite activar el perfil sin el asistente solo cuando la identidad y la topología observada coinciden. Las listas de orden traducen las cuatro posiciones lógicas de cada joystick, declaradas por `gridLayout`, al grupo físico `0..3` del controlador. Deben ser permutaciones completas y calibradas en el dispositivo real.
